...
export function validateContactForm(formData) {
  ...
  if (sanitizedValues.name.length > MAXIMUM_NAME_LENGTH) {
    return { errorMessage: `Name must be less than ${MAXIMUM_NAME_LENGTH} characters.` };
  }
  ...
  if (sanitizedValues.email.length > MAXIMUM_EMAIL_LENGTH) {
    return { errorMessage: `Email must be less than ${MAXIMUM_EMAIL_LENGTH} characters.` };
  }
  ...
  if (sanitizedValues.message.length > MAXIMUM_MESSAGE_LENGTH) {
    return {
      errorMessage: `Message must be less than ${MAXIMUM_MESSAGE_LENGTH} characters.`,
    };
  }
