                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', transformStyle: 'preserve-3d' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', transform: 'translateZ(10px)' }}>
                    {project.links.github && (
                      <a href={project.links.github} target="_blank" rel="noopener noreferrer" aria-label={`${project.title} GitHub repository`} title={`${project.title} GitHub repository`} style={{ color: 'var(--color-text-muted)', transition: 'color 0.2s' }}>
                        <Github size={20} />
                      </a>
                    )}
                    {project.links.live && (
                      <a href={project.links.live} target="_blank" rel="noopener noreferrer" aria-label={`${project.title} live demo`} title={`${project.title} live demo`} style={{ color: 'var(--color-text-muted)', transition: 'color 0.2s' }}>
